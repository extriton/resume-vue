...
<div id="about" class="box left">
    <vue-custom-scrollbar class="box-content" :settings="scrollSettings" ref="scroll-area">
        <h3># {{ person.about.title }}</h3>
        <p class="sub"></p>
        <div v-for="(item, index) in person.about.paragraphs" :key="index">
            <p v-html="item"></p>
        </div>
        <br />
        <h3># {{ person.about.experience.title }}</h3>
        <p class="sub"></p>
        <div v-for="(item, index) in person.about.experience.paragraphs" :key="'a' + index">
            <p v-html="item">{{ item }}</p>
        </div>
        <br />
        <h3># {{ person.about.new_projects.title }}</h3>
        <p class="sub"></p>
        <!-- gallery begin here -->
    ...
    <modal name="project-detail" :width="800" :height="600">
        <div v-if="selectedProject !== null" class="modal-content">
            ...
            <div class="desc">
                ...
                <br />
                Link: <a :href="selectedProject.link" _target="blank">{{ selectedProject.link }}</a>
            </div>
        </div>
    </modal>
</div>
...
export default {
    ...
    computed: {
        projects () {
            const res = []
            for (let i = 0; i < this.person.portfolio.projects.length; i++)
                if (this.person.portfolio.projects[i].isNew)
                    res.push(this.person.portfolio.projects[i])
            return res
        },
        ...mapGetters(['person'])
    },
